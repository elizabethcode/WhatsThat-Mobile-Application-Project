...
export default class UpdateMsg extends Component {
  ...
  deleteMessage = async (message_id) => {  
    try {
      ...
      if (response.ok) {
        const newMessages = this.state.messages.filter((message) => message.message_id !== message_id);
        this.setState({ messages: newMessages });
    ...
    return (
      <View style={styles.messageContainer}>
        ...
        <View style={styles.messageTextContainer}>
          ...
          {item.message}
          <br></br>
        <TouchableOpacity onPress={() => this.updateMessage(item.message_id)}>
        <Ionicons name="create" size={20} color="green" style={styles.deleteIcon} />
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.title}> Update/Manage Chat: {chat_id}</Text>
