...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, KeyboardAvoidingView, FlatList, ActivityIndicator, Modal, Alert } from 'react-native';
...
export default class ChatsSendScreen extends Component {
    constructor(props) {
        super(props);
    ...
    removeAuserFromChat = async (userId) => {
        const { chatId } = this.state;
        try {
            ...
            if (response.status === 200) {
                ...
                // Update the state
                this.setState({ chat: updatedChat });
				alert('Success', 'the user removed successfully.');
				this.props.navigation.goBack()
            }
        } catch (error) {
            console.log(error);
                        ...
                        <View style={styles.modalContainer}>
                            <View style={styles.modalContent}>
                                ...
                                <FlatList
                                    ...
                                    renderItem={({ item }) => (
                                        <View style={styles.memberItem}>
                                            ...
                                            {item.user_id !== userId && (
                                                <TouchableOpacity onPress={() => this.removeAuserFromChat(item.user_id)}>
                                ...
                                <View style={styles.inputContainer}>
                                    ...
                                    <TouchableOpacity style={styles.addButton} onPress={() => this.addAuserToChat(newUserId)}>
